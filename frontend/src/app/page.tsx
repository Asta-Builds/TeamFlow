...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-indigo-500 selection:text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden md:flex items-center gap-6 text-xs font-semibold text-slate-600 dark:text-slate-400">
              ...
              <a href="#agents" className="hover:text-slate-900 dark:hover:text-white transition">Multi-Agent Swarm</a>
      ...
      {/* Hero Section */}
      <section className="relative overflow-hidden pt-16 pb-24 lg:pt-24 lg:pb-32 border-b border-slate-200/80 dark:border-slate-800/80">
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[350px] bg-indigo-600/10 dark:bg-indigo-600/20 blur-[120px] rounded-full pointer-events-none" />

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-8">
          {/* Announcement pill */}
          <div className="inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-50 dark:bg-indigo-950/40 px-3.5 py-1 text-xs font-semibold text-indigo-700 dark:text-indigo-300 shadow-sm backdrop-blur-xs">
          ...
          <div className="max-w-4xl mx-auto space-y-4">
            <h1 className="text-4xl sm:text-6xl font-black tracking-tight text-slate-900 dark:text-white leading-[1.1]">
              ...
              <span className="bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-500 dark:from-indigo-400 dark:via-indigo-200 dark:to-white bg-clip-text text-transparent">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3.5 pt-2">
            ...
            <button
              onClick={loginWithClerk}
              className="inline-flex items-center gap-2 rounded-xl border border-indigo-500/40 bg-white dark:bg-slate-900/90 px-5 py-3 text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition cursor-pointer"
            >
          ...
          <div id="agents" className="max-w-4xl mx-auto pt-10 text-left">
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/90 p-5 sm:p-7 shadow-2xl space-y-6">
              {/* Simulator Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
                ...
                <div className="flex items-center gap-2 text-xs">
                  <span className="font-mono text-[11px] text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800/40 px-2 py-0.5 rounded-md font-semibold">
                    ...
                    <button
                      ...
                      className={`p-3 rounded-xl border text-left transition cursor-pointer ${
                        activeStep === idx
                          ...
                          : "border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 hover:bg-slate-100 dark:hover:bg-slate-800/60"
                      }`}
                    >
                      <div className="flex items-center gap-1.5 text-xs font-bold text-slate-900 dark:text-white truncate">
                        <StepIcon className="h-3.5 w-3.5 text-indigo-500 dark:text-indigo-400 shrink-0" />
                        <span className="truncate">{step.role}</span>
                      </div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5 truncate">{step.action}</div>
                    </button>
                  );
                })}
              </div>
              ...
              <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-900 dark:text-white">{DEMO_STEPS[activeStep].agent}</span>
                    <span className="text-[10px] font-semibold text-indigo-700 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/80 border border-indigo-200 dark:border-indigo-800/60 px-2 py-0.5 rounded-md">
                      {DEMO_STEPS[activeStep].role}
                    </span>
                  </div>

                  <div className="flex items-center gap-3 text-[11px] font-mono text-slate-500 dark:text-slate-400">
                    <span>Tokens: <strong className="text-slate-700 dark:text-slate-200">{DEMO_STEPS[activeStep].tokens}</strong></span>
                    <span>Cost: <strong className="text-emerald-600 dark:text-emerald-400">{DEMO_STEPS[activeStep].cost}</strong></span>
                  </div>
                </div>
                ...
                {DEMO_STEPS[activeStep].pr_url && (
                  <div className="pt-1">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 text-[11px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-800/40 px-2.5 py-1 rounded-md hover:text-indigo-700 dark:hover:text-indigo-300"
                    >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-slate-500 pt-1 border-t border-slate-200 dark:border-slate-800/80">
                <div className="flex items-center gap-2 font-mono text-[11px]">
                  <span className="h-2 w-2 rounded-full bg-emerald-500"></span>
                  <span>Langfuse Session: ticket-124-prod-trace</span>
                </div>
                <div className="flex items-center gap-4 text-[11px]">
                  <span>Total Tokens: <strong className="text-slate-700 dark:text-slate-300">1,940</strong></span>
                  <span>Total Latency: <strong className="text-slate-700 dark:text-slate-300">3.2s</strong></span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 gap-6 lg:grid-cols-4 text-center">
            <div className="space-y-1">
              ...
              <div className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Feature Delivery Speed</div>
            </div>
            <div className="space-y-1">
              <div className="text-3xl sm:text-4xl font-black text-indigo-600 dark:text-indigo-400">99.4%</div>
              <div className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">QA Verification Gate Pass</div>
            </div>
            <div className="space-y-1">
              <div className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white">pgvector</div>
              <div className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Architecture & Code RAG</div>
            </div>
            <div className="space-y-1">
              <div className="text-3xl sm:text-4xl font-black text-emerald-600 dark:text-emerald-400">100%</div>
              <div className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Langfuse Observability</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="features" className="py-20 lg:py-28 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* Card 1 */}
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-6 space-y-3 hover:border-slate-300 dark:hover:border-slate-700 transition shadow-sm">
            ...
            <h4 className="text-base font-bold text-slate-900 dark:text-white">LangGraph Multi-Agent Swarms</h4>
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-6 space-y-3 hover:border-slate-300 dark:hover:border-slate-700 transition shadow-sm">
            ...
            <h4 className="text-base font-bold text-slate-900 dark:text-white">Clerk Identity & Access Management</h4>
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
      ...
      <section id="pricing" className="py-20 lg:py-28 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-7 space-y-6 flex flex-col justify-between shadow-sm">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Ideal for small teams exploring virtual management.</p>
              </div>
              ...
              <ul className="space-y-2.5 text-xs text-slate-700 dark:text-slate-300 pt-2">
                ...
                <li className="flex items-center gap-2"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 dark:text-emerald-400" /> Staging Release Pipelines</li>
              </ul>
            </div>
            <Link
              href="/login"
          ...
          {/* Growth (Featured) */}
          <div className="relative rounded-2xl border-2 border-indigo-500 bg-white dark:bg-slate-900 p-7 space-y-6 flex flex-col justify-between shadow-2xl shadow-indigo-600/10 dark:shadow-indigo-600/20">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Full swarm power for fast-scaling engineering teams.</p>
              </div>
              <div className="flex items-baseline gap-1">
              ...
              <ul className="space-y-2.5 text-xs text-slate-700 dark:text-slate-300 pt-2">
                ...
                <li className="flex items-center gap-2"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 dark:text-emerald-400" /> Production Deploys & Rollback</li>
                <li className="flex items-center gap-2"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 dark:text-emerald-400" /> Langfuse Tracing & Cost Logs</li>
              </ul>
            </div>
            <Link
              href="/login"
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-7 space-y-6 flex flex-col justify-between shadow-sm">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Dedicated security, Clerk Auth, and custom agents.</p>
              </div>
              <div className="flex items-baseline gap-1">
              ...
              <ul className="space-y-2.5 text-xs text-slate-700 dark:text-slate-300 pt-2">
                ...
                <li className="flex items-center gap-2"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 dark:text-emerald-400" /> Clerk Organization & SSO Integration</li>
                <li className="flex items-center gap-2"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 dark:text-emerald-400" /> Unlimited Langfuse Trace Retention</li>
                <li className="flex items-center gap-2"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 dark:text-emerald-400" /> Custom LLM Model Adapters</li>
                <li className="flex items-center gap-2"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 dark:text-emerald-400" /> 99.9% Uptime SLA & Priority Support</li>
              </ul>
            </div>
            <Link
              href="/login"
      ...
      <footer className="border-t border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-950 py-12 text-xs text-slate-600 dark:text-slate-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-6">
            ...
            {SOURCE_REPOSITORY_URL ? <a href={SOURCE_REPOSITORY_URL} target="_blank" rel="noreferrer" className="hover:text-slate-900 dark:hover:text-slate-300 transition">Source repository</a> : null}
