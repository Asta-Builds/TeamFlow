...
export function ValidationContractCard({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/90 shadow-xl overflow-hidden transition-all duration-200">
      {/* Contract Header */}
      <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-gradient-to-r dark:from-slate-900 dark:via-slate-900/95 dark:to-slate-800 border-b border-slate-200 dark:border-slate-800">
          ...
          <div
            className={`h-9 w-9 rounded-xl flex items-center justify-center border shadow-xs ${
              isFullyPassed
                ...
                : "bg-amber-500/10 border-amber-500/30 text-amber-600 dark:bg-amber-950/80 dark:border-amber-700/60 dark:text-amber-400"
            }`}
          >
              ...
              <span
                className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded-full border ${
                  isFullyPassed
                    ...
                    : "bg-amber-500/10 text-amber-700 dark:bg-amber-950/90 dark:text-amber-300 border-amber-500/30 dark:border-amber-700/60"
                }`}
              >
        ...
        <div className="flex items-center gap-2">
          {contract.coverage_percentage !== undefined && (
            <span className="text-[11px] font-mono px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 font-semibold">
              Coverage: <span className="text-indigo-600 dark:text-indigo-400">{contract.coverage_percentage}%</span>
            </span>
          )}
          <button
            type="button"
      ...
      {isExpanded && (
        <div className="p-4 space-y-3">
          ...
          <div className="space-y-1.5 bg-slate-50 dark:bg-slate-950/60 p-3 rounded-xl border border-slate-200 dark:border-slate-800/80">
            <div className="flex justify-between text-[11px] font-mono font-semibold text-slate-500 dark:text-slate-400">
              <span className="flex items-center gap-1.5">
                <ListChecks className="h-3.5 w-3.5 text-indigo-600 dark:text-indigo-400" />
                <span>Verification Score</span>
              </span>
              <span className={isFullyPassed ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-amber-600 dark:text-amber-400 font-bold"}>
            ...
            {items.map((item) => (
              <div
                ...
                className={`group flex items-start gap-3 p-3 rounded-xl border transition-all ${
                  interactive ? "cursor-pointer hover:border-slate-400 dark:hover:border-slate-600" : ""
                } ${
                  item.passed
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-xs font-semibold ${
                        item.passed ? "text-slate-400 dark:text-slate-300 line-through opacity-80" : "text-slate-900 dark:text-white"
                      }`}
                    >
                      {item.label}
                    </span>
                    {item.blocker && (
                      <span className="text-[9px] font-black uppercase px-1.5 py-0.5 rounded bg-rose-50 dark:bg-rose-950 border border-rose-200 dark:border-rose-800/60 text-rose-700 dark:text-rose-300">
                        Blocker
                      </span>
                    )}
                    {item.category && (
                      <span className="text-[9px] font-mono uppercase text-slate-600 dark:text-slate-500 bg-slate-100 dark:bg-slate-900 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-800">
                        {item.category}
                      </span>
                    )}
                  </div>
                  {item.notes && (
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 font-mono">{item.notes}</p>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/80 text-[11px] text-slate-500 font-mono">
