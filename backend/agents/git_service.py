...
def _ensure_origin_configured(
    ...
    if tok:
        remote_url = f"https://x-access-token:{tok}@github.com/{clean_repo}.git"
...
def git_merge_pull_request(
    ...
    clean_repo = (repo or res_repo).replace("https://github.com/", "").replace(".git", "").strip("/")
    if is_protected_repo(clean_repo):
    ...
    if token and clean_repo and "/" in clean_repo and pr_number and pr_number > 0 and (not source_branch or source_branch == target_branch):
        try:
            p_resp = requests.get(
                f"https://api.github.com/repos/{clean_repo}/pulls/{pr_number}",
...
def devops_create_project_repo(
    ...
    if remote_res.get("simulated"):
        return {
            "ok": False,
            "error": "GitHub is not configured for this workspace. Connect a GitHub integration in Settings.",
            "status_code": 503,
        }
